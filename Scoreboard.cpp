...
//    font = displayFont;
    ...
    score_text.setCharacterSize(FONT_SIZE); // set the character size in pixels
    score_text.setFillColor(Color::Yellow); // set the color
    score_text.setPosition(frame_width/2 - tank_w/2+5, 0);
    stream.str("");

    stream << m_lifes ;
    lifes_text.setString(stream.str());
    lifes_text.setCharacterSize(FONT_SIZE); // set the character size in pixels
